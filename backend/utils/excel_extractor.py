...
def _get_df_slice_from_range(df: pd.DataFrame, range_str: str) -> pd.DataFrame:
...
def _extract_chart_data(worksheet: openpyxl.worksheet.worksheet.Worksheet, sheet_df: pd.DataFrame) -> Generator[Tuple[str, Dict[str, Any]], None, None]:
    for chart in worksheet._charts:
        ...
        try:
            source_table_df = _get_df_slice_from_range(sheet_df, total_range_str)
            ...
            if source_table_df.shape[1] > 2:
                ...
                nested_chart_data = transform_grouped_data(source_table_df, grouping_column, value_columns)
            else:
            ...
            print(f"Peringatan: Gagal memproses data chart '{chart.title}' di rentang {total_range_str}. Error: {e}")
            continue

def transform_grouped_data(df: pd.DataFrame, group_by_col: str, value_cols: list) -> dict:
    nested_data = {}
    if group_by_col not in df.columns or not all(c in df.columns for c in value_cols):
        print(f"Peringatan: Kolom yang dibutuhkan tidak ditemukan. Mengembalikan data flat.")
...
def _find_data_tables(sheet_df: pd.DataFrame, processed_ranges: Set[str], sheet_name: str) -> Generator[Dict[str, Any], None, None]:
...
def generate_json_from_excel(file_bytes: bytes, file_name: str) -> Dict[str, Any]:
    file_object = io.BytesIO(file_bytes);
    workbook = openpyxl.load_workbook(file_object, data_only=True)
    result = {
        "fileName": file_name,
        "sheets": []
    }
    ...
    with pd.ExcelFile(file_object) as excel_file:
        for i, sheet_name in enumerate(workbook.sheetnames):
            ...
            for range_str, chart_object in _extract_chart_data(worksheet, sheet_df):
                sheet_data["contentObjects"].append(chart_object)
                processed_ranges.add(range_str)
                
            for table_object in _find_data_tables(sheet_df, processed_ranges, sheet_name):
